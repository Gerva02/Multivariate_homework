...
#prova v2 gervica 
N<-100
norm_samplev2_gervica<- matrix(NA, nrow=N, ncol=4)
for (i in (1:N)){
    M = matrix(NA, nrow=2000, ncol=4)
    for (q in (1:2000)){
       # questo sampla un numero a caso da (1 a 4 )
    Y_hot_i = matrix(NA, nrow=n, ncol=4) # questa è la nx4 dove vanno inseriti i vettori già codificati
    Y_i = sample(4,n,replace =T) #vettore non ancora codificato (contente tutte le 100 estrazioni)
    ...
    norm_samplev2_gervica[i,]<-( colSums(M/2000) - 100* pi_0)*sqrt(2000)
}
...
plot(norm_samplev2_gervica[,1],norm_samplev2_gervica[,2])
